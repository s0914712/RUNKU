...
| 🎻 **星獸音階教室** | 妹妹（樂理，不用單字） | 用鍵盤按出大調與小調音階（上行再下行），同步顯示這個音在小提琴的哪一條弦、按哪一條貼紙 |
...
妹妹專屬的樂理遊戲，不使用單字庫。上面用鋼琴鍵盤按出音階（黑白鍵讓「半音」看得見），
下面同步標出這個音在小提琴的**哪一條弦、按哪一條貼紙**。

### 為什麼這樣排

| 設計 | 原因 |
|------|------|
| 小調用**旋律小音階**，上行升 6、7 音，下行還原 | 小提琴課教的就是這個。做成自然／和聲小調的話，她在琴上拉出來會是錯的 |
| **每個音階都要上行再下行**（8 + 7 = 15 個音） | 旋律小音階上下行的音本來就不一樣，不走完看不出差別；真正練琴也是上下行都拉 |
| 指板畫的是她琴上那**五條貼紙**，不是抽象的圓點 | 和琴上實際的貼法一致，看得懂才用得上 |
| 指板**直的畫**：琴頭在上，弦由左到右 G D A E，一條貼紙橫跨四條弦 | 和琴譜上的指板圖、還有把琴立起來看的樣子一致；琴上本來就是一條膠帶貼過去的 |
| 半音時明講「**二指靠著一指**」並把兩指連起來 | 這是小提琴上半音的實際手感——兩根手指真的貼在一起 |
| 從 **D → G → A 大調**開始，不是從 C | 這三個調從空弦出發；C D E F G A B 是鋼琴的順序，不是小提琴的順序 |
| **21 個音階全部排在第一把位**，🎻 標出琴課範圍 | 螢幕上按到的每個音都能在琴上找到（`scripts/check_music_theory.mjs` 會驗證） |
| 每個音階都**唸得出正確音名** | F 大調的第四音是 B♭ 不是 A♯。程式用「七個音用掉 A~G 各一次」推導，不是用半音數硬湊 |

### 五條貼紙怎麼對應

貼紙在距空弦 +2 / +4 / +5 / +7 / +9 個半音的位置，每條弦都一樣。
以最粗的 G 弦為例，由琴頭往琴身依序是 A① B② C③ D④ E⑤——剛好是從空弦往上數的五個音階音：

| 位置 | 畫面上的說法 |
|------|-------------|
| 空弦 | 不用按手指 |
| 貼紙① / ② / ③ | 一指 / 二指 / 三指，直接按貼紙 |
| 貼紙①和②之間 | **二指靠著一指，兩根手指貼在一起**（這個音沒有貼紙） |
| 貼紙③再往前一點 | 三指往前伸（旋律小音階的升七音就在這裡） |
| 貼紙④ | 四指，聲音和上面那條空弦一樣，可以拿來對音 |
| 貼紙⑤ | 四指再往前伸 |

貼紙④⑤ 畫在每條弦上（琴上本來就貼著），但拉音階時會換到上面那條弦去按——
貼紙④＝上面那條弦的空弦、貼紙⑤＝上面那條弦的貼紙①，所以亮起來的一定是實際會按的那條弦。

> 如果琴上的貼法改了，改 `musicTheory.js` 的 `TAPE_OFFSETS` 一行即可（`TAPE_MARK` 的記號數量要跟著一致）。

### 25 個關卡

```
教學：全音與半音 → D G A C 大調🎻 → A 自然小調🎻 → 🎧 聽辨大調/小調
  → 教學：旋律小音階（上行升 6、7，下行還原）
  → A 旋律🎻 → E 自然/旋律🎻 → D 自然/旋律🎻 → F 大調 → G 自然/旋律🎻
  → 🎧 再聽一次 → B 自然/旋律 → E B 大調 → C 自然/旋律 → F 自然/旋律
```

🎻 = 琴課會拉到的調。按錯不會失敗，只少一點分數；可開「💡 提示」讓下一個正確的鍵發光，
或按「🔊 先聽一次」聽完整的上下行。發音用 Web Audio 即時合成，不需要任何音檔。
