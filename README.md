...
| 指板只畫**三條貼紙**，不是抽象的圓點 | 和她琴上第一把位的貼法一致，看得懂才用得上 |
...
貼紙在距空弦 +2 / +4 / +5 個半音的位置（A 弦上就是 B / C♯ / D）：

| 位置 | 畫面上的說法 |
|------|-------------|
| 空弦 | 不用按手指 |
| 貼紙① / ② / ③ | 一指 / 二指 / 三指，直接按貼紙 |
| 貼紙①和②之間 | **二指靠著一指，兩根手指貼在一起** |
| 貼紙③再往前一點 | 三指往前伸（旋律小音階的升七音就在這裡） |

> 貼紙位置用的是最常見的三貼紙貼法。如果芊芊琴上的貼法不同，改 `musicTheory.js` 的 `TAPE_OFFSETS` 一行即可。
